...
function decorateHtml(html) {
  ...
  // Strip the in-markdown "Table of Contents" section so we can render our own rail.
  // Matches from <h2>Table of Contents</h2> up to (but not including) the next <h2>.
  html = html.replace(
    /<h2[^>]*>\s*Table of Contents\s*<\/h2>[\s\S]*?(?=<h2)/i,
    ''
  );
  ...
  // Inject ids on h2 and collect TOC entries.
  html = html.replace(/<h2(\s[^>]*)?>([\s\S]*?)<\/h2>/g, (match, attrs, inner) => {
...
export function getBriefingBySlug(slug) {
  ...
  const rawHtml = marked(fileContents);
  const { html, toc } = decorateHtml(rawHtml);

  return { slug, ...meta, content: html, toc };
}
